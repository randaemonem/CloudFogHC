...
public class Distances {
	...
	public static int theClosestGateway(List<FogDevice> gatewayDevices, FogDevice sinkMobile){//verify what return type is better (int or ApDevice)		
		int choose=gatewayDevices.get(0).getMyId();//Id of the first gateway in the list
		double min=checkDistance(gatewayDevices.get(0).getCoord(), sinkMobile.getCoord());//the first Ap
		for(FogDevice gateway:gatewayDevices){
			setDistance(checkDistance(gateway.getCoord(), sinkMobile.getCoord()));


			if (getDistance()<min){
				choose=gateway.getMyId();//index or id
				min=getDistance();
			}
		}
	...
	public static int theClosestServerCloudlet(List<FogDevice> serverCloudlets, FogDevice smartThing){
	...
	public static int theClosestServerCloudletToAp(List<FogDevice> serverCloudlets, FogDevice apDevice){
